...
export default function Home() {
  ...
  const handleSetFolder = async () => {
    ...
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to set folder"}`);
    }
  };
  ...
  const handleScanVideos = async () => {
    try {
      ...
      setStatusMessage(`Found ${result.video_count} videos in ${result.show_count} shows`);
      refetch();
      refetchFiles();
      refetchShows();
    } catch (err) {
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to scan"}`);
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const handlePlayFile = async (filePath: string) => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to play"}`);
    }
  };
  ...
  const handleStartStreaming = async () => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to start streaming"}`);
    }
  };
  ...
  const handleSetSubtitleMode = async (mode: SubtitleMode) => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to set subtitle mode"}`);
    }
  };
  ...
  const handleAddToPlaylist = async (showName: string) => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to add to playlist"}`);
    }
  };
  ...
  const handleRemoveFromPlaylist = async (index: number) => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to remove from playlist"}`);
    }
  };
  ...
  const handleMovePlaylistItem = async (index: number, direction: "up" | "down") => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to move item"}`);
    }
  };
  ...
  const handleClearPlaylist = async () => {
    try {
      ...
      setStatusMessage(`Error: ${err instanceof Error ? err.message : "Failed to clear playlist"}`);
    }
  };
        ...
        <section className="space-y-4">
          ...
          <div className="text-sm">
            {config.current_playing ? (
              <p>Now Playing: <span className="text-green-400">{config.current_playing}</span></p>
            ) : (
          ...
          <div className="space-y-2">
            ...
            <div className="max-h-60 overflow-y-auto space-y-1 border border-gray-700 rounded p-2">
              {files && files.files.length > 0 ? (
                files.files.slice(0, 20).map((file, idx) => (
                  <button
                    key={idx}
                ...
                <p className="text-sm text-gray-500 p-2">No files available - scan videos first</p>
              )}
            </div>
          </div>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="max-h-40 overflow-y-auto space-y-1 border border-gray-700 rounded p-2">
              {shows && shows.shows.length > 0 ? (
                ...
                <p className="text-sm text-gray-500 p-2">No shows available - scan videos first</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium">Current Playlist ({config.playlist.length} items):</p>
                ...
                config.playlist.map((item, idx) => (
                  ...
                  >
                    <span className="flex-1 text-sm">
                      {item.show_name}
                      {item.episode_range && ` - Episodes ${item.episode_range[0]}-${item.episode_range[1]}`}
                    </span>
                    <button
